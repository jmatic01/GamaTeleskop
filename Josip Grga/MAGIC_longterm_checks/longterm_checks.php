...
// Dictionary for translating the database parameter names to datacheck limits 
// names
...
$conn = mysql_connect('localhost','DaQ','daK@L9P9') or trigger_error("SQL", E_USER_ERROR);
$db = mysql_select_db('longterm',$conn) or trigger_error("SQL", E_USER_ERROR);
...
$result4 = mysql_query($sql4, $conn) or trigger_error("SQL", E_USER_ERROR);


// process DataCheck limits for the correct telescope, set all the numbers in 
// one array
// M1
  $paramArray = "[";
...
$newthing4 = "[";
// while there are rows to be fetched...
while ($list = mysql_fetch_assoc($result4)) {
